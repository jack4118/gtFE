...
var thArray  = Array(
        ...
        '<?php echo $translations['M03495'][$language]; /* Total Number of Direct Downline */ ?>',
...
function exportBtn() {
    ...
    var formData = {
        command     : "addExcelReq",
        API         : "memberGetRecruitPromoReport",
        fileName    : "recruitPromoReport",
        searchData   : searchData,
        params      : exportParams,
        headerAry   : thArray,
        keyAry      : key,
...
function loadDefaultListing(data, message) {
    ...
    var tableNo;
    if(data){
        if(data.recruitPromoReport){
            var newList = [];
            $.each(data.recruitPromoReport, function(k, v) {
                var action = `
                ...
                var rebuildData = {
                    date                                : v['date'],
                    totalNumberOfDirectDownline         : action,
                    totalPVP                            : numberThousand(v['totalPVP'],2),
                    bonusPayout                         : numberThousand(v['bonusPayout'],2),
                };
...
function pagingCallBack(pageNumber, fCallback){
    ...
    var formData = {
        command    : "memberGetRecruitPromoReport",
        searchData  : searchData,
        pageNumber : pageNumber
    };
    if(!fCallback)
