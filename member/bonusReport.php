...
<section class="pageContent loginPagePadding">
    <div class="">
        <div class="pageTitle">
            <?php echo $translations['M03320'][$language]; //Performance Bonus Report ?>
        </div>
        <div class="mt-4">
        ...
        if (list){
            $.each(list, function(k, v){
                var rebuildData = {
                    ...
                    bonusValue      : addCommas(Number(v['bonusValue']).toFixed(2)),  
                    percentage      : addCommas(Number(v['percentage']).toFixed(2)),
                    payableAmount   : addCommas(Number(v['payableAmount']).toFixed(2)),
                    cvRate          : addCommas(Number(v['cvRate']).toFixed(2)),
                    bonusPayout     : addCommas(Number(v['bonusPayout']).toFixed(2))
                };
