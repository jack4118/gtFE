                                    ...
                                    <div class="promoProducts_item">
                                        ...
                                        <div class="row">
                                            <div class="col-md-4 form-group">
                                                ...
                                                <select id="product_id" class="form-control" onchange="loadSalePrice(this)"></select>
                                            </div>
...
var pricelistId         = '<?php echo $_POST['id'] ?>';
var wrapperLength = 2;
...
function viewPricelistDetail() {
    var formData = {
        command         : 'viewPricelistDetail',
        pricelist_id    : pricelistId,
    };

    var fCallback = loadPriceListDetail;
    ajaxSend(url, formData, method, fCallback, debug, bypassBlocking, bypassLoading, 0);
}

function loadPriceListDetail(data, message) {
    if(data) {
        $('#promoProducts').empty();

        var pricelist = data.pricelist;
        $('#pricelist_name').val(pricelist.pricelist_name);
        ...
        var pricelistDetail = data.pricelistDetail;
        if(pricelistDetail && pricelistDetail.length > 0) {
            $.each(pricelistDetail, function(k, v) {
                ...
                var html = `
                    <div class="promoProducts_item">
                        ...
                        <div class="row">
                            ...
                            <input id="condition" type="hidden" class="form-control" value="${v['condition']}">
                        </div>
                    </div>
                `;
                ...
                $('#end_date').val(v['end_date']);
                $('.promoProducts_item:last-child').find('#product_id').val(v['product_id']).trigger('change');
                $('.promoProducts_item:last-child').find('#discount_amount').val(v['discount']);
                $('.promoProducts_item:last-child').find('#discount_type').val(v['discount_type']).trigger('change');
            });
        }
    }
}
...
function loadSalePrice(e) {
    ...
    var products;
    if(condition == 'productCategory' && productsByCategory && productsByCategory.length > 0) products = productsByCategory;
    ...
    if(data) {
        ...
        if(productList && productList.length > 0) {
            $.each(productList, function(k, v) {
                ...
                $('#promoProducts').append(html);
                $('.promoProducts_item:last-child').find('#product_id').trigger('change');
...
function editPriceList() {
    ...
    var formData = {
        command         : "editPriceList",
        pricelist_id    : pricelistId, 
        pricelist_name  : $("#pricelist_name").val(),
        activeStatus    : 1,
        product_list    : productList,
    };
    var fCallback = successEditPriceList;
    ajaxSend(url, formData, method, fCallback, debug, bypassBlocking, bypassLoading, 0);
}

function successEditPriceList(data, message) {
    showMessage('Price List Updated Successfully', 'success', 'Success Update Price List', 'check', 'pricelisting.php');
}
