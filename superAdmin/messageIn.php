                                    ...
                                    <div id="collapseOne" class="panel-collapse collapse in" role="tabpanel" aria-labelledby="headingOne" aria-expanded="true">
                                        ...
                                        <div id="searchMsg" class="text-center alert" style="display: none;"></div>
                                            <form id="searchMessageSent" role="form">
                                                ...
                                                <div class="col-sm-4 form-group">
                                                    <label class="control-label" for="" data-th="sender">Sender</label>
                                                    <input type="text" class="form-control" dataName="sender" dataType="text">
                                                </div>
                                                <div class="col-sm-4 form-group">
                                                    <label class="control-label" for="" data-th="name">Provider Name</label>
                                                    <input type="text" class="form-control" dataName="providerName" dataType="text">
                                                </div>
                            ...
                            <form>
                                <div id="basicwizard" class="pull-in">
                                    <div class="tab-content b-0 m-b-0 p-t-0">
                                        ...
                                        <div id="messageInListDiv" class="table-responsive"></div>
                                        <span id="paginateText"></span>
                                        <div class="text-center">
                                            <ul class="pagination pagination-md" id="pagerMessageInList"></ul>
                                        </div>
                                    </div>
                                </div>
                            </form>
...
<?php include("shareJs.php"); ?>
    ...
    <script>
        ...
        var divId = 'messageInListDiv';
        var tableId = 'messageInListTable';
        var pagerId = 'pagerMessageInList';
        var btnArray = Array();
        ...
        $(document).ready(function() {
            ...
            var formData = {command: 'getMessageInList', pageNumber : pageNumber};
        ...
        function loadDefaultListing(data, message) {
            var tableNo;
            buildTable(data.messageData, tableId, divId, thArray, btnArray, message, tableNo);
            pagination(pagerId, data.pageNumber, data.totalPage, data.totalRecord, data.numRecord);
            if(data.messageData) {
                $.each(data.hiddenData, function(key, val) {
                    $.each(val, function(innerKey, innerVal) {
                        if(key == "Content") storeContent(innerVal);
                    });
                });
                ...
                $('#'+divId).find('tr').each(function() {
                    ...
                    tr.find('td:eq(2)').addClass('text-center').text('').append('<i class="fa fa-file-text fa-2"></i>');
                    tr.find('td:eq(2) i').attr('id', tdDataInID);
                    tr.find('td:eq(2) i').addClass('cursorPointer');
                    tr.find('td:eq(2) i').on('click tap', function() {
                        showData(this.id);
                    });
        ...
        function loadDelete(data, message) {
            loadDefaultListing(data, message);
            $('#messageSentMsg').addClass('alert-success').html("<span>Message sent deleted successfully.</span>").show();
            setTimeout(function() {
                $('#messageSentMsg').removeClass('alert-success').html('').hide(); 
            }, 3000);
        }
