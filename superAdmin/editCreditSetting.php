...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#editCreditBtn').click(function() {
            ...
            $('#creditSettingList input[name=setting]').each(function() {
                id.push(this.id);
                values.push(this.value);
            });

            $('#creditSettingList input[name=admin]').each(function() {
                var adminFlag;
                ...
                admin.push(adminFlag);
            });

            $('#creditSettingList input[name=member]').each(function() {
                var memberFlag;
                if(this.checked == true){
                    memberFlag = "1";
                }else{
    ...
    function loadEdit(data, message) {
        ...
        if(typeof(creditData) != 'undefined') {
            ...
            $.each(creditData['name'], function(key, val) {
                ...
                if(creditData['admin'][key] == 1){
                    var admin = '<input type="checkbox" name="admin" value="'+creditData['admin'][key]+'" checked> Admin';
                }else{
                    var admin = '<input type="checkbox" name="admin" value="'+creditData['admin'][key]+'"> Admin';
                }
                
                if(creditData['member'][key] == 1){
                    var member = '<input type="checkbox" name="member" value="'+creditData['member'][key]+'" checked> Member';
                }else{
                    var member = '<input type="checkbox" name="member" value="'+creditData['member'][key]+'"> Member';
                }
                
                $('#creditSettingList').append('<div class="form-group"><label class="control-label">' + val + '</label><input name="setting" id="' + id + '" type="text" class="form-control" value="' + value + '">'+admin+' &nbsp '+member+'</div>');
            });
        }
        else
    ...
    function sendEdit(data, message) {
        showMessage('Credit Setting successfully updated.', 'success', 'Edit credit Setting', 'credit-card', 'credit.php');
    }
</script>
</body>
</html>