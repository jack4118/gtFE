                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="card-box p-b-0">
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <form role="form" id="editInternalAccountForm" data-parsley-validate novalidate>
                                        <input type="hidden" name="command" value="editInternalAccountData">
                                        <input type="hidden" id="internalAccountId" name="internalAccountId" value="">
                                            ...
                                            <!-- <div class="form-group">
                                                ...
                                                <input id="description" type="textarea" cols="3" rows="3" name="description" class="form-control">
                                            </div> -->
                        ...
                        <div class="col-md-12 m-b-20">
                            ...
                            <button id="editInternalAccount" type="submit" class="btn btn-primary waves-effect waves-light">Confirm</button>
                        </div>
                    </div>
                    <!-- End row -->
                </div> <!-- container -->
...
<script>
    ...
    $(document).ready(function() {
        var internalAccountId = window.location.search.substr(1).split("=");
        internalAccountId = internalAccountId[1];
        $("#internalAccountId").val(internalAccountId);

        if(internalAccountId != '') {
            var formData = {
                'command': 'getInternalAccountData',
        ...
        $('#editInternalAccount').click(function() {
            var validate = $('#editInternalAccountForm').parsley().validate();
                if(validate) {
                    fCallback = sendEdit;
                    var formData = $('#editInternalAccountForm').serialize();
                    ajaxSend(url, formData, method, fCallback, debug, bypassBlocking, bypassLoading, 0);
                }
        });
    });
    ...
    function loadEdit(data, message) {
        $.each(data.internalAccData, function(key, val) {
    ...
    function sendEdit(data, message) {
        showMessage('Interal Account successfully updated.', 'success', 'Edit Interal Account', 'InternalAccount', 'internalAccounts.php');
    }
</script>
</html>